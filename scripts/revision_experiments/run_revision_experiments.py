...
def compute_mesh_and_eigen(name_or_params, use_cache: bool = True):
    ...
    if isinstance(name_or_params, str):
        if use_cache:
            # Prefer the .npz cache; fall back to the committed .json. The nine paper
            # meshes ship only as .json (the .npz is git-ignored and regenerated by
            # step1), so a fresh clone reproduces E1-E4 from the .json rather than
            # recomputing. This matters for double_torus: the paper's 4846-vertex
            # ablation basis exists in the repo only as double_torus.json.
            cache = cache_dir / f"{name_or_params}.npz"
            cache_json = cache_dir / f"{name_or_params}.json"
            if cache.exists():
                d = np.load(cache)
            ...
            if cache_json.exists():
                ...
                print(f"  WARNING: {name_or_params}: using float32 .json (approximate); "
                      f".npz gives the paper's float64 values", file=sys.stderr)
                d = _json.loads(cache_json.read_text())
                V = np.asarray(d["vertices"], dtype=np.float64)
                F = np.asarray(d["faces"], dtype=np.int32)
                lam = np.asarray(d["eigenvalues"], dtype=np.float64)[:N_MODES]
                vec = np.asarray(d["eigenvectors"], dtype=np.float64)[:N_MODES, :]
                return V, F, lam, vec
...
def mean_metrics_over_frames(V, F, coef_list, eigvecs):
    ...
    return {
        ...
        # Retained per-frame values. The E3 comparison turns on the *shape* of this
        # distribution, not its mean: under freqmatch the sphere is bimodal across
        # frames, which is why its std exceeds its mean. Summary statistics alone
        # cannot show that, and the supplementary histogram is built from these.
        "beta0_frames": [int(b) for b in beta0s],
    }
