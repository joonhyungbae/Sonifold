"""
Generate Fig.3: Nodal-surface gallery — four shapes (sphere, cube, torus, double torus)
× three audio (A1, A3, A5). Matches paper caption: sphere (symmetry), cube (angular),
torus (genus 1), double torus (genus 2). Direct mapping; diverging colormap; nodal (|f|<eps) in black.
Run from project root: python figures/fig3_nodal_gallery.py
Output: figures/fig3_nodal_gallery.pdf
"""
...
N_EIGEN = 50
# Four shapes per paper caption: sphere, cube, torus, double torus
MESHES = ["sphere", "cube", "torus", "double_torus"]
AUDIOS = ["A1", "A3", "A5"]
...
def compute_scalar_for(V, F, evecs, evals, audio_id: str):
    """Return scalar_values for the given mesh data and audio."""
...
def main():
    ...
    out_path = root / "figures" / "fig3_nodal_gallery.pdf"
    out_path.parent.mkdir(parents=True, exist_ok=True)

    # Precompute: load each mesh once, then scalar fields for each audio
    data_grid = []
    ...
    # Diverging colormap (RdBu), symmetric around 0
    try:
        cmap = cm.colormaps["RdBu_r"]
    except (AttributeError, KeyError):
        cmap = plt.cm.RdBu_r
    n_rows, n_cols = len(MESHES), len(AUDIOS)
    fig = plt.figure(figsize=(9, 3.2 * n_rows), dpi=FIG_DPI)
    for i, mesh_name in enumerate(MESHES):
        for j, audio_id in enumerate(AUDIOS):
            ...
            ax.set_aspect("equal")
            ax.set_title(f"{mesh_name}, {audio_id}", fontsize=9)
