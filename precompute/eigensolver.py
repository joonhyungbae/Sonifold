...
def _deterministic_v0(n: int) -> np.ndarray:
    """Reproducible Arnoldi starting vector. A constant vector is in the
    kernel of the cotangent Laplacian (1 is harmonic on a closed surface),
    so we use a fixed pseudo-random vector that is orthogonal to constants
    in expectation. Seeding with a fixed RNG makes eigsh runs identical
    across processes and across mesh rescalings (revision round 1)."""
...
def _solve_cpu(A: csr_matrix, k: int):
    tol = _eigsh_tol()
    # maxiter: allow ARPACK enough iterations on larger meshes at tight tol.
    maxiter = max(A.shape[0] * 20, 300000)
    v0 = _deterministic_v0(A.shape[0])
    w, v = scipy_eigsh(
        A, k=k, which="SA", tol=tol, maxiter=maxiter,
...
def _solve_gpu(A: csr_matrix, k: int):
    ...
    v0_gpu = cp.asarray(_deterministic_v0(A.shape[0]))
    w_gpu, v_gpu = cusplinalg.eigsh(
        A_gpu, k=k, which="SA", tol=tol,
        return_eigenvectors=True, v0=v0_gpu,
    )
