...
def _double_torus() -> Tuple[np.ndarray, np.ndarray]:
    """Double torus (genus 2). Requires data/meshes/double_torus.obj (χ = -2)."""
    path = _data_dir() / "double_torus.obj"
    if not path.exists():
        raise FileNotFoundError(
            f"Double torus mesh not found at {path}. "
            "Run scripts/analysis/genus2_mesh_check_and_fix.py to generate a "
            "verified χ=-2 mesh. Silent fallback to a single torus has been "
            "removed to prevent the published 'double torus' from secretly "
            "being a genus-1 surface."
        )
